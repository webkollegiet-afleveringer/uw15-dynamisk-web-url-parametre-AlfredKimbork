    ...
    .then(data => {
        main.innerHTML = `
            <figure class="destination__figure">
                <img class="destination__img" src="./img/${data.image}" alt="${data.title}">
                <figcaption class="destination__btn">
                    <button class="btn--like" id="destination-${data.id}"">
            ...
            <section class="destination__content">
                <h1 class="content__location">${data.destination}</h1>
                <h2 class="content__title">${data.title}</h2>
                <span class="content__subtitle">${data.subtitle}</span>
                <p class="content__text">${data.text}</p>
                <h3 class="facilities__heading">Faciliteter</h3>
                <ul class="facilities__list">${data.facilities.map(facility => {return `<li class="facilities__item">${facility}</li>`}).join("")}</ul>
            </section>
        `

        document.querySelector(".btn--like").addEventListener("click", handleLike)
